...
public:
    ...
    static int encodeJsonToBinary(const JsonDocument& doc, uint8_t* buffer, int maxLen) {
        ...
        for (JsonPairConst kv : root) {
            ...
            buffer[idx++] = DT_KEY_TOKEN; buffer[idx++] = keyToken;
            JsonVariantConst val = kv.value();
            if (val.is<const char*>()) {
                const char* s = val.as<const char*>();
                uint8_t valToken = getTokenId(s);
                if (valToken != 0xFF) { buffer[idx++] = DT_VAL_TOKEN; buffer[idx++] = valToken; }
                else {
                    int slen = strlen(s);
                    if (idx + 2 + slen < maxLen) {
                        buffer[idx++] = DT_VAL_RAW_STR; buffer[idx++] = (uint8_t)slen;
                        memcpy(buffer + idx, s, slen); idx += slen;
                    }
                }
                 ...
                 if (f == (int32_t)f) {
                     ...
                     if (i32 >= -128 && i32 <= 127) { buffer[idx++] = DT_VAL_INT8; buffer[idx++] = (int8_t)i32; }
                     else if (i32 >= -32768 && i32 <= 32767) { buffer[idx++] = DT_VAL_INT16; int16_t t = (int16_t)i32; memcpy(buffer+idx, &t, 2); idx+=2; }
                     else { buffer[idx++] = DT_VAL_INT32; memcpy(buffer+idx, &i32, 4); idx+=4; }
                 } else { buffer[idx++] = DT_VAL_FLOAT; memcpy(buffer+idx, &f, 4); idx+=4; }
            }
        }
        buffer[idx++] = DT_END;
        return idx;
    ...
    static void decodeBinaryToJson(const uint8_t* buffer, int len, JsonDocument& doc) {
        ...
        while (idx < len) {
            ...
            if (type == DT_KEY_TOKEN) currentKey = getString(buffer[idx++]);
            else if (currentKey) {
                switch (type) {
                    case DT_VAL_TOKEN: doc[currentKey] = getString(buffer[idx++]); break;
                    case DT_VAL_INT8: doc[currentKey] = (int8_t)buffer[idx++]; break;
                    case DT_VAL_INT16: { int16_t v; memcpy(&v, buffer+idx, 2); idx+=2; doc[currentKey] = v; break; }
                    case DT_VAL_INT32: { int32_t v; memcpy(&v, buffer+idx, 4); idx+=4; doc[currentKey] = v; break; }
                    case DT_VAL_FLOAT: { float v; memcpy(&v, buffer+idx, 4); idx+=4; doc[currentKey] = (float)((int)(v*100+0.5))/100.0; break; }
                    case DT_VAL_RAW_STR: { uint8_t l = buffer[idx++]; char t[256]; if(idx+l<=len){memcpy(t,buffer+idx,l); t[l]=0; doc[currentKey]=String(t); idx+=l;} break; }
                }
                currentKey = nullptr; 
            }
        }
